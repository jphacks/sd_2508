...
[![IMAGE ALT TEXT HERE](image/mimoca.png)](https://www.youtube.com/watch?v=lA9EluZugD8)
...
近年、保育園の子供バス置き去りによる熱中症・死亡事故や保育士不足による監視の目の不足が起きている。このシステムではスマートフォンを持たない幼児や高齢者といった方々を、保護者がより安心して、また遠隔から見守ることができる。
...
- **屋内検知**: Fingerprinting法によるBLE三点測位で、数十cm単位の高精度な室内位置推定
- **バス置き去り検知**: BLEビーコン1台で移動物内の置き去りを検知
- **屋外追跡**: LoRaWAN通信とGPSを用いて、外出後でも位置情報を低消費電力で定期送信
- **Webアプリでの一元管理**: 屋内・屋外の位置データを統合し、地図や平面図上にリアルタイムで可視化
...
2. **バス置き去り検知モード（機能2）**: BLEビーコン1台による移動物内での置き去り検知
3. **屋外GPS追跡モード（機能3）**: GPSによる親子トラッカー間の距離監視
...
- **バスへの置き去り検知**: BLE検知範囲の監視により、車内に一人取り残されている状態を3分以内に検出
- **屋外での見失い防止**: GPS追跡により、親子間の距離が設定値（デフォルト30m）を超えた際に警告
...
#### UI/UX面
- **pomunity.comを参考にした洗練されたデザイン**: 直感的で使いやすいインターフェース
